-- ====================================================
-- Fix: Agregar parámetro p_price a add_to_cart para soportar precios con descuento desde el frontend
-- ====================================================
...
DROP FUNCTION IF EXISTS add_to_cart(INTEGER, UUID, VARCHAR(255), INTEGER, JSONB, DECIMAL(10,2), DECIMAL(10,2), INTEGER, INTEGER);

-- Actualizar función add_to_cart con parámetro p_price
CREATE OR REPLACE FUNCTION add_to_cart(
  ...
  p_price DECIMAL(10,2) DEFAULT NULL,  -- Nuevo parámetro para precio desde frontend (con descuento ya aplicado)
...
BEGIN
  ...
  -- Usar el precio proporcionado desde el frontend (ya con descuento aplicado) o el de la base de datos
  ...
  ELSE
    -- Si no existe, insertar nuevo item usando v_final_price (precio con descuento ya aplicado)
    ...
    VALUES (
      ...
      p_quantity, v_final_price,  -- Usar v_final_price en lugar de v_menu_item.price
...
COMMENT ON FUNCTION add_to_cart IS 'Agrega un item al carrito o incrementa la cantidad si ya existe. Soporta precio opcional desde el frontend (con descuento aplicado).';
