-- Fix: Usar nombres exactos de servicios
-- flex-bill, pick-n-go, tap-order-pay
...
BEGIN
  ...
  -- CORREGIDO: Usar nombres exactos de servicios
  ...
  -- Debug: Agregar info sobre servicios detectados
  RAISE NOTICE 'Services detected - table(flex-bill): %, tap(tap-order-pay): %, pickgo(pick-n-go): %, raw services: %',
    has_table_service, has_tap_service, has_pickgo_service, client_services;
  ...
  RAISE NOTICE 'Final customer count: %', customer_count;
