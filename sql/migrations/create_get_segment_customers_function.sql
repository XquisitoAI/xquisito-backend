-- =====================================================
-- MIGRATION: Create get_segment_customers RPC function
-- Author: Claude Code Assistant
-- Date: 2025-12-18
-- Description: Creates RPC function to return actual customer
--              data for a segment (not just count)
-- =====================================================

CREATE OR REPLACE FUNCTION get_segment_customers(
...
BEGIN
  ...
  WHERE 1=1
    ...
    -- Solo usuarios con phone o email (para poder contactarlos)
    AND (p.phone IS NOT NULL OR p.email IS NOT NULL);
END;
$$ LANGUAGE plpgsql;

-- Add comment
COMMENT ON FUNCTION get_segment_customers IS 'Returns actual customer data (user_id, phone, email, first_name) for a segment based on filters. Used for campaign sending.';
