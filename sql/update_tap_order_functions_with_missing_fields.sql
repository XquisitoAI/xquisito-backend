-- ====================================================
-- Script para actualizar funciones Tap Order con campos faltantes
-- Agregar soporte para: images, custom_fields, extra_price
-- ====================================================

-- Función 1: Actualizar create_tap_order_with_first_dish para incluir campos faltantes
...
-- Función 2: Actualizar add_dish_to_existing_tap_order para incluir campos faltantes
...
-- Función 3: Actualizar la función de cálculo de total para incluir extra_price
...
'Crea tap_order y primer dish_order con soporte completo para images, custom_fields y extra_price';

COMMENT ON FUNCTION add_dish_to_existing_tap_order IS
'Agrega platillo a tap_order existente con soporte completo para images, custom_fields y extra_price';

COMMENT ON FUNCTION update_tap_order_total IS
'Recalcula total de tap_order incluyendo extra_price de cada dish_order';

-- Script de verificación final
DO $$
BEGIN
    RAISE NOTICE '=== ACTUALIZACIÓN FUNCIONES TAP ORDER ===';
    RAISE NOTICE '✅ create_tap_order_with_first_dish actualizada con images, custom_fields, extra_price';
    RAISE NOTICE '✅ add_dish_to_existing_tap_order actualizada con images, custom_fields, extra_price';
    RAISE NOTICE '✅ update_tap_order_total actualizada para incluir extra_price en cálculos';
    RAISE NOTICE 'Listo para testing con campos completos';
END $$;