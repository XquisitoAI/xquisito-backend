...
class AnalyticsService {
  ...
  async getCompleteDashboardData(filters) {
    try {
      ...
      throw new Error(
        `Error fetching complete dashboard data: ${error.message}`,
      );
    }
  }
  ...
  processDashboardData(rawData, granularity) {
    ...
    const { metricas, grafico, filtros_aplicados, tiempo_promedio_mesa } =
      rawData;
  ...
  async getDashboardMetricsAllServices(filters) {
    ...
    try {
      ...
      const sqlServiceType = service_type
        ? SERVICE_NAME_MAP[service_type] || service_type
        : null;

      const { data, error } = await supabase.rpc(
        "get_dashboard_metrics_all_services",
        {
          p_restaurant_id: restaurant_id || null,
      ...
      throw new Error(
        `Error fetching dashboard metrics (all services): ${error.message}`,
      );
    }
  }
  ...
  processAllServicesData(rawData, granularity) {
    ...
    return {
      ...
      articulo_mas_vendido: articulo_mas_vendido || {
        nombre: "Sin datos",
        unidades_vendidas: 0,
      },
      filtros_aplicados: filtros_aplicados || {},
