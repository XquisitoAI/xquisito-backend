...
class CampaignsService {
    ...
    async createCampaign(campaignData, userId) {
        try {
            ...
            await this.validateSegmentOwnership(campaignData.segment_id, campaignData.restaurant_id);
            ...
            const { data: campaign, error } = await this.supabase
                ...
                .insert({
                    ...
                    total_targeted: await this.calculateTargetedCustomers(campaignData.segment_id)
                })
                .select()
                .single();
    ...
    async updateCampaign(campaignId, updateData, restaurantId) {
        try {
            ...
            if (updateData.segment_id && updateData.segment_id !== campaign.segment_id) {
                updateData.total_targeted = await this.calculateTargetedCustomers(updateData.segment_id);
            }
    ...
    async deleteCampaign(campaignId, restaurantId) {
        try {
            ...
            if (campaign.status === 'running') {
                throw new Error('No se puede eliminar una campaña que está en ejecución');
            }
     ...
     * @param {string} campaignId - UUID de la campaña
     * @param {Array} templates - Array de templates { template_id, template_type, is_primary }
     * @param {number} restaurantId - ID del restaurante (para validación)
    ...
    async getCampaignTemplates(campaignId) {
        try {
            ...
            // Formatear la respuesta para incluir los datos del template correcto
            return data.map(ct => ({
                id: ct.id,
                ...
                template_data: ct.template_type === 'sms' ? ct.sms_templates : ct.email_templates
            }));
        } catch (error) {
    ...
    async getCampaignSendStats(campaignId) {
        try {
            ...
            data.forEach(send => {
                ...
                stats.by_delivery_method[send.delivery_method] = (stats.by_delivery_method[send.delivery_method] || 0) + 1;
            });
    ...
    validateCampaignData(campaignData) {
        const required = ['restaurant_id', 'name', 'segment_id', 'reward_type', 'reward_value', 'start_date', 'end_date'];
        ...
        // Validar reward_value
        if (campaignData.reward_value <= 0) {
            throw new Error('El valor de la recompensa debe ser mayor a 0');
        }

        // Validar delivery_methods
        if (campaignData.delivery_methods) {
            const validMethods = ['email', 'sms', 'whatsapp', 'push'];
            const invalidMethods = campaignData.delivery_methods.filter(method => !validMethods.includes(method));

            if (invalidMethods.length > 0) {
                throw new Error(`Métodos de entrega inválidos: ${invalidMethods.join(', ')}`);
            }
        }
    }
    ...
    async validateSegmentOwnership(segmentId, restaurantId) {
        ...
        if (error || !data) {
            throw new Error('El segmento no existe o no pertenece a este restaurante');
        }
    }
    ...
    async calculateTargetedCustomers(segmentId) {
        try {
            ...
            if (error || !data) {
                console.warn('Could not calculate targeted customers for segment:', segmentId);
                return 0;
            }
    ...
    async getCampaignAnalytics(restaurantId) {
        try {
            ...
            const analytics = {
                ...
                total_customers_reached: data.reduce((sum, c) => sum + (c.total_sent || 0), 0),
                total_redemptions: data.reduce((sum, c) => sum + (c.total_redeemed || 0), 0),
                average_open_rate: data.length > 0 ?
                    data.reduce((sum, c) => sum + (c.open_rate || 0), 0) / data.length : 0,
                average_redemption_rate: data.length > 0 ?
                    data.reduce((sum, c) => sum + (c.redemption_rate || 0), 0) / data.length : 0,
                campaigns: data
            };
