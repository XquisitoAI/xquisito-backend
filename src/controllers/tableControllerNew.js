...
class TableController {
    ...
    async createDishOrder(req, res) {
        try {
            const { tableNumber } = req.params;
            const { userId, guestName, item, quantity = 1, price } = req.body;
    ...
    async checkTableAvailability(req, res) {
        try {
            const { tableNumber } = req.params;
            const table = await tableService.checkTableAvailability(parseInt(tableNumber));
    ...
    async getSplitPaymentStatus(req, res) {
        try {
            const { tableNumber } = req.params;
            const splitStatus = await tableService.getSplitPaymentStatus(parseInt(tableNumber));

            const summary = {
                total_people: splitStatus.length,
                paid_people: splitStatus.filter(p => p.status === 'paid').length,
                pending_people: splitStatus.filter(p => p.status === 'pending').length,
                total_collected: splitStatus.reduce((sum, p) => sum + parseFloat(p.amount_paid || 0), 0),
                total_remaining: splitStatus.reduce((sum, p) => sum + parseFloat(p.remaining || 0), 0)
            };
    ...
    async getActiveUsers(req, res) {
        try {
            const { tableNumber } = req.params;
            const activeUsers = await tableService.getActiveUsers(parseInt(tableNumber));
