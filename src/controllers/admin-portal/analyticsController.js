const analyticsService = require("../../services/admin-portal/analyticsService");
...
class AnalyticsController {
    ...
    try {
      ...
      if (!validAgeRanges.includes(filters.age_range)) {
        return res.status(400).json({
          success: false,
          error:
            "Rango de edad inválido. Debe ser: todos, 14-17, 18-25, 26-35, 36-45, 46+",
        });
      }
  ...
  async getCompleteDashboardData(req, res) {
    try {
      ...
      console.log(
        "🎯 [getCompleteDashboardData] Filtros recibidos:",
  ...
  async getAllSellingItems(req, res) {
    try {
      const filters = {
        restaurant_id: req.query.restaurant_id
          ? parseInt(req.query.restaurant_id)
      ...
      const data = await analyticsService.getAllSellingItems(filters);
      res.json({ success: true, data, timestamp: new Date().toISOString() });
    } catch (error) {
      console.error("Error in getAllSellingItems controller:", error);
      res.status(500).json({
        success: false,
        error: error.message,
        timestamp: new Date().toISOString(),
  ...
  async getDashboardMetricsAllServices(req, res) {
    try {
      ...
      if (
        ...
        !validServiceTypes.includes(filters.service_type)
      ) {
        return res.status(400).json({
          success: false,
          error:
            "Tipo de servicio inválido. Debe ser: flex-bill, pick-n-go, tap-order-pay, tap-pay, room-service, o null para todos",
        });
      }

      console.log(
        "🎯 [getDashboardMetricsAllServices] Filtros recibidos:",
        JSON.stringify(filters, null, 2),
      );

      const data =
        await analyticsService.getDashboardMetricsAllServices(filters);
      ...
      console.error(
        "Error in getDashboardMetricsAllServices controller:",
        error,
      );
      res.status(500).json({
        success: false,
    ...
    try {
      ...
      try {
        ...
        tableStructures = {
          ...
          detailed_order: sampleDetailedOrder?.[0]
            ? Object.keys(sampleDetailedOrder[0])
            : [],
