const tapDishOrderService = require("../../services/tap-order-and-pay/tapDishOrderService");
...
class TapDishOrderController {
  ...
  async createOrderWithFirstDish(req, res) {
    try {
      ...
      const customerData = {
        clerk_user_id: dishData.clerk_user_id || dishData.guest_id,
  ...
  async getDishOrders(req, res) {
    try {
      ...
      const result =
        await tapDishOrderService.getDishOrdersByTapOrder(tapOrderId);
