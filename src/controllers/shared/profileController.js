...
class ProfileController {
  ...
  async createOrUpdateProfile(req, res) {
    try {
      ...
      const { data: existingProfile, error: findError } = await getDb()
        .from("profiles")
        .select("*")
        .eq("id", authUser.id)
        .single();
      ...
      if (existingProfile) {
        ...
        const { data: updatedProfile, error: updateError } = await getDb()
          .from("profiles")
          .update(updateData)
          .eq("id", authUser.id)
          .select()
        ...
        const { data: newProfile, error: createError } = await getDb()
          .from("profiles")
          .insert(insertData)
          .select()
          .single();
  ...
  async uploadProfilePhoto(req, res) {
    try {
      ...
      const ImageUploadService = require("../../services/shared/imageUploadService");
